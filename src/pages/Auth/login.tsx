...
const Login = () => {
  ...
  const kiemTraCa = async (user: any) => {
    try {
      ...
      if (caHienTai.nguoidungid === user.id) {
        // Ca của chính mình → vào luôn
        navigate('/tables');
        return;
      }

      // Ca của người khác chưa đóng → hiện dialog chọn
      setCaMode('caCu');
      setCaCu(caHienTai);
      setCaDialog(true);
    } catch {
      // Lỗi kiểm tra ca → cho vào nhưng hiện cảnh báo
      navigate('/tables');
    }
  };
  ...
  const handleMoCaMoi = async () => {
    setSavingCa(true);
    try {
      // Nếu có ca cũ chưa đóng → đóng trước
      if (caCu) {
  ...
  const handleTiepTucCaCu = () => {
    // Tiếp tục ca cũ (chỉ áp dụng nếu là ca của người khác trong cùng ngày)
    setCaDialog(false);
    navigate('/tables');
  };
  ...
  return (
    <div className="min-h-screen flex align-items-center justify-content-center p-3"
      ...
      <Dialog
        ...
        visible={caDialog}
        style={{ width: 420 }}
        closable={false}  // Không cho đóng dialog — bắt buộc phải xử lý
        onHide={() => {}}
      >
